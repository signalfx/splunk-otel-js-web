...
function assertSessionIsEmpty() {
	try {
		if (localStorage['_splunk_rum_sid_test']) {
			throw new Error('Session is expected to be empty, but is set in localStorage.')
		}

		if (document.cookie.indexOf('_splunk_rum_sid_test') >= 0) {
			throw new Error(`Session is expected to be empty, but is set in cookies: ${document.cookie}`)
		}
	} finally {
		delete localStorage['_splunk_rum_sid_test']
		document.cookie = '_splunk_rum_sid_test=;expires=Thu, 01 Jan 1970 00:00:00 GMT'
	}
}
