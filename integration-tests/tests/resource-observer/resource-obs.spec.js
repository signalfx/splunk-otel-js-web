...
module.exports = {
  ...
  'resources can be ignored': async function(browser) {
    ...
    await browser.assert.not.ok(browser.globals.getReceivedSpans().find(span => span.tags['http.url'] === url + 'test.js'));
    const imgSpan = browser.globals.getReceivedSpans().find(span => span.tags['http.url'] === url + 'splunk-black.png');
