...
describe('can produce websocket events', () => {
	...
	it('can produce a series of spans from basic usage', async () => {
		...
		wsSpans.forEach((span) => {
			expect('ws://127.0.0.1:8979/').toBe(span.attributes['url.full'])
		})
		...
		expect(wsSpans[1].attributes['http.request.body.size']).toBe(12)
		...
		expect(wsSpans[2].attributes['http.response.body.size']).toBe(8)
	...
	it('can report failed connect to non-listening port', async () => {
		...
		expect(wsSpans[0].attributes['url.full']).toBe('ws://127.0.0.1:31874/')
