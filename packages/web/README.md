...
| **Instrumentations**                  |                                                       |          |                                                                                                    |                                                                                                                                                                                                                                                                                                                                                                         |
| `instrumentations.blockingElement`    | `boolean\|Config`                                     | ❌       | `false` (`true` if `spaMetrics.monitors` includes `elements` and `blockingSelectors` is non-empty) | Per-element blocking spans for elements matching `spaMetrics.blockingSelectors`. `enabled` overrides the spaMetrics-derived default; there is no separate selector config. See [SPA Metrics](#spa-metrics) below.                                                                                                                                                       |
