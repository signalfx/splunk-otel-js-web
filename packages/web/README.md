...
When `experimental: true`, `instrumentations.loaf` is enabled, and the browser supports `long-animation-frame`, the default `longtask` instrumentation is suppressed for that page session to avoid double-reporting overlapping main-thread work. With the experimental flag off or in browsers without LoAF support, `longtask` remains the fallback unless it is explicitly disabled.
