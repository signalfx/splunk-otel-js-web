...
| `spaMetrics`                          | `boolean\|Config`                                     | ❌       | `true`                                                                                             | SPA page completion metrics. Supports global settings and ordered `urlOverrides` for page-specific `clearLoadingResourcesOnNewPage`, `quietTime`, `ignoreUrls`, `blockingSelectors`, `maxPageLoadTimeoutForManualApi`, `maxPageLoadWaitTime`, `maxResourcesToWatch`, and `monitors`.                                                                                    |
...
| **Instrumentations**                  |                                                       |          |                                                                                                    |                                                                                                                                                                                                                                                                                                                                                                         |
| `instrumentations.blockingElement`    | `boolean\|Config`                                     | ❌       | `false` (`true` if `spaMetrics.monitors` includes `elements` and `blockingSelectors` is non-empty) | Per-element blocking spans for elements matching `spaMetrics.blockingSelectors`. Requires `experimental: true`; `enabled` overrides the spaMetrics-derived default, and there is no separate selector config. See [SPA Metrics](#spa-metrics) below.                                                                                                                    |
...
Selectors always come from `spaMetrics.blockingSelectors` — there is no separate selector config for this instrumentation. If `instrumentations.blockingElement` is unset, it's enabled only when `elements` is included in `spaMetrics.monitors` and `blockingSelectors` is non-empty. Set `instrumentations.blockingElement: { enabled: true }` to enable it regardless of `spaMetrics.monitors`, or `instrumentations.blockingElement: false` to disable it independent of `spaMetrics`. Selectors are resolved once when the instrumentation is enabled and do not re-resolve on later SPA route changes, even if a matching `spaMetrics.urlOverrides` entry would otherwise apply.
