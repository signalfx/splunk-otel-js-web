...
| **Instrumentations**                  |                                                       |          |                                                                                                    |                                                                                                                                                                                                                                                                                                                                                                         |
| `instrumentations.blockingElement`    | `boolean\|Config`                                     | ❌       | `false` (`true` if `spaMetrics.monitors` includes `elements` and `blockingSelectors` is non-empty) | Per-element blocking spans for elements matching configured CSS selectors. Falls back to `spaMetrics.blockingSelectors` when unset. See [SPA Metrics](#spa-metrics) below.                                                                                                                                                                                              |
...
If `instrumentations.blockingElement` is unset, it falls back to `spaMetrics.blockingSelectors` (enabled only when `elements` is included in `spaMetrics.monitors` and `blockingSelectors` is non-empty). Set `instrumentations.blockingElement.selectors` to use different selectors than the PCT `elements` monitor, or `instrumentations.blockingElement: false` to disable it independent of `spaMetrics`. Selectors are resolved once when the instrumentation is enabled and do not re-resolve on later SPA route changes, even if a matching `spaMetrics.urlOverrides` entry would otherwise apply.
