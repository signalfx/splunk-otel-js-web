...
| `navigationMetrics`                   | `boolean\|Config`                                     | ❌       | `true`                                                                                                    | Page completion metrics for document loads and route changes. Supports global settings and ordered `urlOverrides` for page-specific `clearLoadingResourcesOnNewPage`, `quietTime`, `ignoreUrls`, `blockingSelectors`, `maxPageLoadTimeoutForManualApi`, `maxPageLoadWaitTime`, `maxResourcesToWatch`, and `monitors`. `spaMetrics` is a deprecated alias for this option. |
| `globalAttributes`                    | `Attributes`                                          | ❌       | `{}`                                                                                                      | Attributes added to every span                                                                                                                                                                                                                                                                                                                                            |
| `dataAttributesToCapture`             | `string[]`                                            | ❌       | `[]`                                                                                                      | Explicit allowlist of `data-*` attributes to capture from click and rage-click targets, and expose to the element picker. Accepts hyphenated names like `data-test-id` or camelCase names like `testId`. Captured values may be sensitive or high-cardinality, so only allowlist attributes intended for telemetry.                                                       |
...
| **Instrumentations**                  |                                                       |          |                                                                                                           |                                                                                                                                                                                                                                                                                                                                                                           |
| `instrumentations.blockingElement`    | `boolean\|Config`                                     | ❌       | `false` (`true` if `navigationMetrics.monitors` includes `elements` and `blockingSelectors` is non-empty) | Per-element blocking spans for elements matching `navigationMetrics.blockingSelectors`. Requires `experimental: true`; `enabled` overrides the navigationMetrics-derived default, and there is no separate selector config. See [Navigation Metrics](#navigation-metrics) below.                                                                                          |
...
The `navigationMetrics` option (deprecated alias: `spaMetrics`) controls page completion time (PCT) calculation for document loads and SPA route changes. When `navigationMetrics` is `true`, the default monitor types are enabled with the default timing settings. Use `navigationMetrics.monitors` globally, or inside `navigationMetrics.urlOverrides`, to control which resource sources can keep PCT waiting for a specific page.
...
The `elements` monitor is opt-in. Add `'elements'` to `navigationMetrics.monitors` and configure `blockingSelectors` to wait for visible loading indicators. Hidden elements, elements with `display: none`, and elements with `visibility: hidden` or `collapse` do not block PCT.
...
Selectors always come from `navigationMetrics.blockingSelectors` — there is no separate selector config for this instrumentation. If `instrumentations.blockingElement` is unset, it's enabled only when `elements` is included in `navigationMetrics.monitors` and `blockingSelectors` is non-empty. Set `instrumentations.blockingElement: { enabled: true }` to enable it regardless of `navigationMetrics.monitors`, or `instrumentations.blockingElement: false` to disable it independent of `navigationMetrics`. Selectors and enablement re-resolve on every SPA route change, so a matching `navigationMetrics.urlOverrides` entry takes effect immediately after navigating — this requires `instrumentations.interactions` to be enabled, since that's what detects SPA route changes (via `history.pushState`/`replaceState` and `hashchange`) and triggers re-evaluation. If `interactions` is disabled, blockingElement still applies whichever config matched the initial page, but never re-resolves after that.

The tab being hidden (`document.visibilityState` turning `'hidden'`) also interrupts every open `blockingElement` span, ending it with `completion: 'visibility_hidden'` before the buffered spans are flushed — this ensures elements still visible when a tab is hidden or the browser discards it aren't silently lost. If the tab becomes visible again and the element is still present, tracking resumes with a fresh span.

If an element matches more than one configured selector, it still gets exactly one span — `browser.element.selector` lists every matched selector as a comma-separated value (in the order the element first matched each one), reflecting every selector the element matched at any point while the span was open — including one dropped by a later `navigationMetrics.urlOverrides` change — not just the one active when it completed. Concurrently open spans are capped at 1000 elements; once reached, additional elements are silently dropped from tracking until an existing one completes, with a single warning logged. Each span also includes `browser.element.xpath`, an XPath locator for the element.

Every span ends with a `browser.element.completion` attribute: `completed` (the element disappeared normally), `interrupted` (the instrumentation was disabled, or the page was unloaded, while the span was still open), `visibility_hidden` (the tab was hidden while the span was still open, see above), or `timeout` (the element stayed visible longer than `maxElementSpanDuration`, default 180000ms/3 minutes — configurable via `instrumentations.blockingElement: { maxElementSpanDuration: <ms> }`). The timeout guards against an element that never disappears producing no telemetry at all.

### Long Animation Frames

When enabled, the `loaf` instrumentation reports browser Long Animation Frames as `long-animation-frame` spans. It is the preferred path for long task visibility, replacing the deprecated `longtask` instrumentation where browser support is available. It is opt-in because browser support is currently Chromium-only.
...
LoAF spans include frame timing attributes such as `loaf.duration`, `loaf.blocking_duration`, `loaf.paint_time`, `loaf.presentation_time`, `loaf.render_start`, `loaf.style_and_layout_start`, and `loaf.first_ui_event_timestamp`. They also include bounded script attribution: `loaf.script_count` reports the original number of scripts, while only the top three scripts by duration are exported as `loaf.script[0..2].*` attributes, including script timing fields such as `duration`, `start_time`, `offset`, `execution_start`, `pause_duration`, `forced_style_and_layout_duration`, and `source_char_position`. `start_time` preserves the browser's performance-origin-relative script timestamp, while `offset` reports the script start relative to the LoAF span start. Script source attribution is reported as provided by the browser; use `exporter.onAttributesSerializing` if your application needs to redact or transform those attributes before export.
		...
		urlOverrides: [
			{
				...
				match: '/cart/',
				quietTime: 1000,
				// Override arrays replace inherited arrays, so repeat global navigation metric ignores if needed.
