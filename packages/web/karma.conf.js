...
module.exports = async function (config) {
	const { nodeResolvePlugin, commonjsPlugin } = await import('../../rollup.shared.mjs')
	...
	config.set({
		...
		websocketServer: {
			...
			beforeStart: (server) => {
				server.on('request', (request) => {
					const cx = request.accept(
						request.requestedProtocols ? request.requestedProtocols[0] : null,
						request.origin,
					)
		...
		rollupPreprocessor: {
			plugins: [
				...
				rollupPolyfills({
					// Needed to make sure even upper dir node_modules are included
					include: [/node_modules/],
				}),
				istanbulrollup({
					exclude: [/node_modules/, 'karma.conf.js', 'rollup.shared.js', 'dist/**'],
				}),
			],
			input: 'test/index.ts',
			output: {
