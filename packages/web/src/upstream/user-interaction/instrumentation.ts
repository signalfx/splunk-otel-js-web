...
> extends InstrumentationBase<T> {
	...
	private eventNames: Set<EventName>

	// for event bubbling
	private eventsSpanMap: WeakMap<Event, api.Span> = new WeakMap<Event, api.Span>()

	private shouldPreventSpanCreation: ShouldPreventSpanCreation

	private spansData = new WeakMap<api.Span, SpanData>()

	// for addEventListener/removeEventListener state
	private wrappedListeners = new WeakMap<
		EventListenerOrEventListenerObject,
		Map<string, Map<Element, EventListenerOrEventListenerObject>>
	>()

	private zonePatched?: boolean

	constructor(config: T) {
		super('@opentelemetry/instrumentation-user-interaction', VERSION, config)
		this.eventNames = new Set(config?.eventNames ?? DEFAULT_EVENT_NAMES)
		this.shouldPreventSpanCreation =
	 ...
	 */
	override disable() {
		const targets = this.getPatchableEventTargets()
	...
	override enable() {
		...
		targets.forEach((target) => {
			...
			this._wrap(target, 'addEventListener', this.patchAddEventListener())
			this._wrap(target, 'removeEventListener', this.patchRemoveEventListener())
		})
	...
	private addPatchedListener(
		...
		let listener2Type = this.wrappedListeners.get(listener)
		if (!listener2Type) {
			listener2Type = new Map()
			this.wrappedListeners.set(listener, listener2Type)
		}
	 ...
	 */
	private allowEventName(eventName: EventName): boolean {
	...
	private createSpan(
		...
		if (!this.allowEventName(eventName)) {
			return undefined
		}
		...
		try {
			...
			if (this.shouldPreventSpanCreation(eventName, element, span) === true) {
				return undefined
			}

			this.spansData.set(span, {
				taskCount: 0,
			})
	 ...
	 */
	private getPatchableEventTargets(): EventTarget[] {
		return window.EventTarget ? [EventTarget.prototype] : [Node.prototype, Window.prototype]
	}

	// utility method to deal with the Function|EventListener nature of addEventListener
	private invokeListener(listener: EventListenerOrEventListenerObject, target: any, args: [evt: Event]): any {
		...
		return (original: EventTarget['addEventListener']) =>
			function addEventListenerPatched(
				...
				const patchedListener = function (this: Element, ...args: [evt: Event]) {
					...
					if (event) {
						parentSpan = instrumentation.eventsSpanMap.get(event)
					}
					...
					const span = instrumentation.createSpan(target, type, parentSpan)
					if (span) {
						if (event) {
							instrumentation.eventsSpanMap.set(event, span)
						}

						return api.context.with(api.trace.setSpan(api.context.active(), span), () => {
							const result = instrumentation.invokeListener(listener, this, args)
							// no zone so end span immediately
							span.end()
							return result
						})
					} else {
						return instrumentation.invokeListener(listener, this, args)
					}
				}
	...
	private patchHistoryApi() {
		...
		this._wrap(history, 'replaceState', this.patchHistoryMethod())
		this._wrap(history, 'pushState', this.patchHistoryMethod())
		this._wrap(history, 'back', this.patchHistoryMethod())
		this._wrap(history, 'forward', this.patchHistoryMethod())
		this._wrap(history, 'go', this.patchHistoryMethod())
	}
	...
	private patchHistoryMethod() {
		...
		return (original: any) =>
			function patchHistoryMethod(this: History, ...args: unknown[]) {
				...
				if (url !== urlAfter) {
					instrumentation.updateInteractionName(urlAfter)
				}
	...
	private removePatchedListener(
		...
		const listener2Type = this.wrappedListeners.get(listener)
		if (!listener2Type) {
			return undefined
		}
	 ...
	 * @param url
	 */
	private updateInteractionName(url: string) {
