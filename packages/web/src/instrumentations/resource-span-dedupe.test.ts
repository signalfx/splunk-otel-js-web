...
import {
	didResourceStartAfterDocumentLoad,
	markResourceHandledByDocumentLoad,
	wasResourceHandledByDocumentLoad,
} from './resource-span-dedupe'
...
describe('resource span ownership', () => {
	...
	it('allows post-load instrumentation to consume a document-load resource once', () => {
		const resource = createResourceEntry(100)

		markResourceHandledByDocumentLoad(resource)

		expect(wasResourceHandledByDocumentLoad(resource)).toBe(true)
		expect(wasResourceHandledByDocumentLoad(resource)).toBe(false)
	})
})
