...
export class SessionManager {
	...
	start() {
		...
		this.attachSessionStorageAndExpirationWatch()
	...
	private attachSessionStorageAndExpirationWatch() {
	...
	private synchronizeStorageWithCurrentSessionState = () => {
		...
		if (!SessionManager.canContinueUsingSession(this.session)) {
			...
			if (this.session.state !== newState && this.session.state !== 'native') {
				this.session = {
					...this.session,
					state: newState,
				}
