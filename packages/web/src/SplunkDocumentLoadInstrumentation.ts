...
import { ATTR_HTTP_RESPONSE_STATUS_CODE, SemanticAttributes } from '@opentelemetry/semantic-conventions'
			...
			if (span && exposedSpan.name !== AttributeNames.DOCUMENT_LOAD) {
				...
				if (exposedSpan.name === AttributeNames.DOCUMENT_FETCH && performance.getEntriesByType) {
					...
					if (navEntries[0]?.responseStatus) {
						span.setAttribute(ATTR_HTTP_RESPONSE_STATUS_CODE, navEntries[0].responseStatus)
					}
				}
