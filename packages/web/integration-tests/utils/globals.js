	...
	return {
		...
		assertNoErrorSpans: async () => {
			await browser.assert.ok(
				backend.getReceivedSpans().every((s) => s.name !== 'onerror'),
				'Ensuring no errors were reported.',
			)
		},
...
module.exports = {
	...
	beforeEach: async (browser, done) => {
		...
		browser.globals.buildInstrumentationBackend = () =>
			buildInstrumentationBackend({
		...
		browser.timesMakeSense = async function (eventsArr, startName, endName) {
			...
			await this.assert.ok(
				name2time[startName] <= name2time[endName],
				`Checking sanity of ${startName} vs ${endName}`,
			)
