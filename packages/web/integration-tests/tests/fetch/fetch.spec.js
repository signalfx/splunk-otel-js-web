...
module.exports = {
	...
	'span created for fetch includes all properties': async function (browser) {
		...
		const fetchSpan = await browser.globals.findSpan(
			(span) => span.tags['http.url'] && span.tags['http.url'].endsWith('/some-data'),
		)
		...
		if (
			browser.options.desiredCapabilities.browserName !== 'Safari' &&
			browser.options.desiredCapabilities.browser_version !== '10.1'
		) {
	...
	'fetch request can be ignored': async function (browser) {
		...
		await browser.assert.not.ok(
			browser.globals
				.getReceivedSpans()
				.find((span) => span.tags['http.url'] && span.tags['http.url'].endsWith('/some-data')),
		)
		await browser.assert.not.ok(
			browser.globals
				.getReceivedSpans()
				.find((span) => span.tags['http.url'] && span.tags['http.url'].endsWith('/no-server-timings')),
		)
	},
	'fetch reported over CORS': async function (browser) {
		const backend2 = await browser.globals.buildInstrumentationBackend()
		await browser.url(
			backend2.getUrl('/fetch/fetch.ejs', undefined, { beaconPort: browser.globals.httpPort }).toString(),
		)
