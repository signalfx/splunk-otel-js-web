...
module.exports = {
	...
	'setting session cookie should work': async function (browser) {
		...
		const parent = await browser.globals.findSpan(
			(span) => span.name === 'documentFetch' && span.tags['location.href'].includes('cookies.ejs'),
		)
		...
		const iframe = await browser.globals.findSpan(
			(span) => span.name === 'documentFetch' && span.tags['location.href'].includes('iframe.ejs'),
		)
	...
	'setting session cookie in iframe should work': async function (browser) {
		...
		const fetchSpan = await browser.globals.findSpan(
			(span) => span.name === 'documentFetch' && span.tags.app === 'iframe',
		)
