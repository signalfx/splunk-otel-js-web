...
module.exports = {
	...
	'XHR request is registered': async function (browser) {
		...
		const xhrSpan = await browser.globals.findSpan(
	...
	'module can be disabled': async function (browser) {
		...
		browser.assert.ok(
			!!browser.globals.getReceivedSpans().find((span) => span.tags['http.url'] === awaitedUrl),
			'Checking presence of xhr span.',
		)
