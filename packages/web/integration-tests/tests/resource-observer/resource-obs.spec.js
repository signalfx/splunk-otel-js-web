...
module.exports = {
	...
	'should report resource loads happening after page load': async function (browser) {
		...
		const plAgentSpans = browser.globals
			.getReceivedSpans()
			.filter((span) => span.tags['http.url'] && span.tags['http.url'].endsWith('splunk-otel-web.js'))
		const plImageSpans = browser.globals
			.getReceivedSpans()
			.filter((span) => span.tags['http.url'] && span.tags['http.url'].endsWith('no-cache.png'))
	...
	'resources can be ignored': async function (browser) {
		...
		await browser.assert.not.ok(
			browser.globals
				...
				.find(
					(span) =>
						span.tags['http.url'] === url + 'test.js' &&
						span.tags['component'] === 'splunk-post-doc-load-resource',
				),
		)
		const imgSpan = browser.globals
			.getReceivedSpans()
			.find(
				(span) =>
					span.tags['http.url'] === url + 'splunk-black.png' &&
					span.tags['component'] === 'splunk-post-doc-load-resource',
			)
	...
	'should create one span for cached resource': async function (browser) {
		...
		const imageSpans = await browser.globals
			.getReceivedSpans()
			.filter((span) => span.tags['http.url'] && span.tags['http.url'].endsWith('splunk-black.png'))
	...
	'should propagate tracing context to created spans': async function (browser) {
		...
		const plImageRootSpan = await browser.globals
			.getReceivedSpans()
			.find(
				(span) =>
					span.tags['http.url'] &&
		...
		const plImageParentSpan = await browser.globals.getReceivedSpans().find((span) => span.name === 'image-parent')
		const plImageChildSpan = await browser.globals
			.getReceivedSpans()
			.find(
				(span) =>
					span.tags['http.url'] &&
		...
		const plScriptRootSpan = await browser.globals
			.getReceivedSpans()
			.find(
				(span) =>
					span.tags['http.url'] &&
		...
		const plScriptParentSpan = await browser.globals
			.getReceivedSpans()
			.find((span) => span.name === 'script-parent')
		const plScriptChildSpan = await browser.globals
			.getReceivedSpans()
			.find(
				(span) =>
					span.tags['http.url'] &&
