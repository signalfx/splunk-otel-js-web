...
module.exports = {
	...
	'DOM resource 4xx': async function (browser) {
		...
		await browser.assert.ok(
			tags['target_src'].endsWith('/nonexistent.png'),
			`Checking target_src: ${tags['target_src']}`,
		)
	},
	'JS syntax error': async function (browser) {
		...
		await browser.assert.strictEqual(
			tags['error.object'],
			isBrowser(browser, 'internet explorer') ? 'String' : 'SyntaxError',
		)
	...
	'JS unhandled error': async function (browser) {
		...
		switch (browser.options.desiredCapabilities.browserName.toLowerCase()) {
			...
			case 'internet explorer':
				await browser.assert.strictEqual(
					tags['error.message'],
					"Unable to set property 'prop1' of undefined or null reference",
				)
				break
		}
	},
	...
	'module can be disabled': async function (browser) {
		...
		browser.assert.ok(
			!!browser.globals.getReceivedSpans().find(({ name }) => name === 'onerror'),
			'Checking presence of error span.',
		)
	...
	'minified script with source map id': async function (browser) {
		...
		await browser.assert.ok(
			errorSpan1.tags['error.source_map_ids'].includes(
		...
		await browser.assert.ok(
			errorSpan2.tags['error.source_map_ids'].includes(
