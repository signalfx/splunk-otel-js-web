...
module.exports = {
	'webvitals spans': async function (browser) {
		...
		browser.assert.ok(
			!!browser.globals.getReceivedSpans().find((span) => span.tags.lcp !== undefined),
			'Checking presence of lcp span.',
		)
		browser.assert.ok(
			!!browser.globals.getReceivedSpans().find((span) => span.tags.cls !== undefined),
			'Checking presence of cls span.',
		)
