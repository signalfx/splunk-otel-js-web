...
module.exports = {
	...
	'documentFetch, resourceFetch, and documentLoad spans': async function (browser) {
		...
		const scriptFetch = await browser.globals.findSpan(
			(span) => span.name === 'resourceFetch' && span.tags['http.url'].includes('splunk-otel-web.js'),
		)
		const brokenImgFetch = await browser.globals.findSpan(
			(span) => span.name === 'resourceFetch' && span.tags['http.url'].includes('/nosuchimage.jpg'),
		)
		...
		if (!browser.globals.isBrowser({ safari: true, ie: true })) {
			await browser.assert.ok(
				docFetch.tags['http.response_content_length'] >= 0,
				'Checking response_content_length',
			)
		...
		if (!browser.globals.isBrowser({ safari: true, ie: true })) {
			await browser.assert.ok(
				scriptFetch.tags['http.response_content_length'] >= 0,
				'Checking response_content_length',
			)
			await browser.assert.ok(docFetch.tags['link.traceId'], 'Checking presence of link.traceId')
			await browser.assert.ok(docFetch.tags['link.spanId'], 'Checking presence of link.spanId')
	...
	'ignoring resource URLs': async function (browser) {
		...
		await browser.assert.not.ok(
			browser.globals
				.getReceivedSpans()
				.find((span) => span.tags['http.url'] === url + 'non-impactful-resource.jpg'),
		)
	},
	'module can be disabled': async function (browser) {
		...
		await browser.assert.not.ok(
			browser.globals.getReceivedSpans().find((span) => SPAN_TYPES.includes(span.name)),
