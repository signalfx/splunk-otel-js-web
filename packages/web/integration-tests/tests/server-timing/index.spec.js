...
module.exports = {
	'traceId should be attached to documentFetch span if Server-Timing was sent': async function (browser) {
		...
		const docFetchSpan = await browser.globals.findSpan(
			(span) => span.name === 'documentFetch' && span.tags['link.traceId'] === expectedTraceId,
		)

		await browser.assert.strictEqual(docFetchSpan.tags['link.traceId'], expectedTraceId)
		await browser.assert.strictEqual(
			docFetchSpan.tags['location.href'],
			browser.globals.getUrl('/server-timing/index.ejs'),
		)
