...
module.exports = {
	...
	'reports a longtask': async function (browser) {
		...
		if (browserIsCompatible(browser)) {
			...
			await browser.assert.strictEqual(
				['self', 'unknown'].includes(longtaskSpan.tags['longtask.name']),
				true,
				'longtask.name',
			)
			await browser.assert.strictEqual(
				longtaskSpan.tags['longtask.entry_type'],
				'longtask',
				'longtask.entry_type',
			)
			await browser.assert.strictEqual(
				longtaskSpan.tags['longtask.attribution.name'],
				'unknown',
				'longtask.attribution.name',
			)
			await browser.assert.strictEqual(
				longtaskSpan.tags['longtask.attribution.entry_type'],
				'taskattribution',
				'longtask.attribution.entry_type',
			)
			await browser.assert.strictEqual(
				longtaskSpan.tags['longtask.attribution.start_time'],
				'0',
				'longtask.attribution.start_time',
			)
			await browser.assert.strictEqual(
				longtaskSpan.tags['longtask.attribution.duration'],
				'0',
				'longtask.attribution.duration',
			)
			await browser.assert.strictEqual(
				longtaskSpan.tags['longtask.attribution.container_type'],
				'window',
				'longtask.attribution.container_type',
			)
			await browser.assert.strictEqual(
				longtaskSpan.tags['longtask.attribution.container_src'],
				'',
				'longtask.attribution.container_src',
			)
			await browser.assert.strictEqual(
				longtaskSpan.tags['longtask.attribution.container_id'],
				'',
				'longtask.attribution.container_id',
			)
			await browser.assert.strictEqual(
				longtaskSpan.tags['longtask.attribution.container_name'],
				'',
				'longtask.attribution.container_name',
			)
			...
			await browser.assert.strictEqual(
				longtaskSpan.duration,
				duration * 1000,
				'Span duration matches longtask duration',
			)
		}
	...
	'can be disabled': async function (browser) {
		...
		browser.assert.ok(
			!!browser.globals.getReceivedSpans().find(({ name }) => name === 'longtask'),
			'Checking presence of longtask span.',
		)
		await browser.globals.assertNoErrorSpans()
