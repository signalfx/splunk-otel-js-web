...
></script>
<script
	src="https://cdn.observability.splunkcloud.com/o11y-gdi-rum/v3.0/splunk-otel-web-session-recorder.js"
	crossorigin="anonymous"
></script>
```
...
></script>
<script
	src="https://cdn.observability.splunkcloud.com/o11y-gdi-rum/v3.0.0/splunk-otel-web-session-recorder.js"
	crossorigin="anonymous"
	integrity="sha384-<integrity>"
></script>
```
...
<html>
	<head>
		...
		<script
			src="https://cdn.observability.splunkcloud.com/o11y-gdi-rum/v1/splunk-otel-web-session-recorder.js"
			crossorigin="anonymous"
		></script>
...
| `persistFailedReplayData`    | `boolean \| 'indexeddb' \| 'localstorage'` | ❌       | `true`   | Persistence strategy for failed uploads (see below)        |
...
| Value            | Description                                                                                                                                                                      |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `true`           | Default. Uses the session replay script's built-in IndexedDB persistence and sends replay logs as OTLP/protobuf. Segments are stored in IndexedDB and retried on next page load. |
| `'localstorage'` | Failed OTLP log exports are queued in localStorage (2 MB budget) and retried on next page load.                                                                                  |
| `'indexeddb'`    | Same as true.                                                                                                                                                                    |
...
// It will load an 3rd party iframe (background service) from this domain 'https://cdn.observability.splunkcloud.com/o11y-gdi-rum/<version>/background-service.html'
