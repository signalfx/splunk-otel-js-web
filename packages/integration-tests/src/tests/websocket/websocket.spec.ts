...
test.describe('websockets', () => {
	...
	test('sending send and on message create a span', async ({ recordPage }) => {
		...
		expect(messageSpans[0].tags['url.full']).toBe('ws://localhost:3000/ws')
		expect(messageSpans[0].tags['http.response.body.size']).toBe('14')
		...
		expect(sendSpans[0].tags['url.full']).toBe('ws://localhost:3000/ws')
		expect(sendSpans[0].tags['http.request.body.size']).toBe('12')
	})
