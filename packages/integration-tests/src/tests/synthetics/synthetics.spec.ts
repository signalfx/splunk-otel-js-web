...
test.describe('synthetics', () => {
	test('applies synthetics run Id attribute on all spans', async ({ recordPage }) => {
		const syntheticsRunId = 'abcd1234'.repeat(4)
		await recordPage.goTo('/synthetics/index.ejs?syntheticsRunId=' + syntheticsRunId)
	...
	test('applies synthetics run id attribute on all spans', async ({ recordPage }) => {
		await recordPage.goTo('/synthetics/index.ejs')
