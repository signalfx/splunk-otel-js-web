...
test.describe('forms', () => {
	test('handles form submit', async ({ recordPage }) => {
		...
		expect(submitSpans[0]).toHaveSpanAttribute('component', 'user-interaction')
		expect(submitSpans[0]).toHaveSpanAttribute('event_type', 'submit')
		expect(submitSpans[0]).toHaveSpanAttribute('target_element', 'FORM')
		expect(submitSpans[0]).toHaveSpanAttribute('target_xpath', '//*[@id="form1"]')
