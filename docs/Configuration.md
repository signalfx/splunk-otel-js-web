...
|`cookieDomain`|`string`|`(none)`|Configure the domain [used for session tracking](Cookies.md). For example if you have sites `foo.example.com` and `bar.example.com`, setting this to `example.com` allows both sites to use same session identifier.|
